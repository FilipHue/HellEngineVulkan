	...
	namespace graphics
	{
		...
		void VulkanSwapchain::CreateImageViews(const VulkanInstance& instance, VulkanDevice& device)
		{
			for (VkImage image : m_images)
			{
				...
				CreateImageView(instance, device, image, m_format.format, VK_IMAGE_ASPECT_COLOR_BIT, image_view);
		...
		void VulkanSwapchain::Init(const VulkanInstance& instance, VulkanDevice& device, VulkanSurface& surface, VkExtent2D current_extent)
		{
			...
			if (!DetectDepthFormat(device))
			{
				...
				HE_CORE_ERROR("Failed to detect depth format");
			}

			m_depth_image.Create(instance, device, VK_IMAGE_TYPE_2D, device.GetDepthFormat(), m_extent.width, m_extent.height, 1, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_IMAGE_TILING_OPTIMAL, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			CreateImageView(instance, device, m_depth_image.GetHandle(), device.GetDepthFormat(), VK_IMAGE_ASPECT_DEPTH_BIT, m_depth_image.GetImageView());
		}
