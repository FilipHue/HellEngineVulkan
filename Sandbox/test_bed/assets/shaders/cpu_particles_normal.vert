...
void main(void) 
{
	vec3 vertexPosition = vec3(ubo.model *  vec4(inPosition, 1.0));
	...
	vec3 lightDist = ubo.lightPos.xyz - inPosition;
	outLightVecB.x = dot(inTangent.xyz, lightDist);
	outLightVecB.y = dot(biTangent, lightDist);
	outLightVecB.z = dot(inNormal, lightDist);

	outViewVec.x = dot(inTangent.xyz, inPosition);
	outViewVec.y = dot(biTangent, inPosition);
	outViewVec.z = dot(inNormal, inPosition);
